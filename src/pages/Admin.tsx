    ...
    <div className="bg-white dark:bg-gray-950 min-h-screen transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex items-center justify-between mb-12">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{t.nav.admin}</h1>
          <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-xl transition-colors">
            <button 
              onClick={() => setActiveTab('jobs')}
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'jobs' ? 'bg-white dark:bg-gray-700 shadow-sm text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}
            >
              {t.nav.jobs}
            </button>
            <button 
              onClick={() => setActiveTab('courses')}
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'courses' ? 'bg-white dark:bg-gray-700 shadow-sm text-orange-600 dark:text-orange-400' : 'text-gray-500 dark:text-gray-400'}`}
            >
              {t.nav.courses}
            </button>
            <button 
              onClick={() => setActiveTab('universities')}
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'universities' ? 'bg-white dark:bg-gray-700 shadow-sm text-teal-600 dark:text-teal-400' : 'text-gray-500 dark:text-gray-400'}`}
            >
              {t.nav.universities}
            </button>
          </div>
        ...
        {activeTab === 'jobs' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white dark:bg-gray-900 p-8 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm sticky top-24 transition-colors">
                <h2 className="text-xl font-bold mb-6 flex items-center dark:text-white">
                  <Plus className="w-5 h-5 mr-2 text-blue-600 dark:text-blue-400" /> {language === 'en' ? 'Add New Job' : language === 'ru' ? 'Добавить вакансию' : 'Yangi ish qo‘shish'}
                </h2>
                <form onSubmit={handleAddJob} className="space-y-4">
                  <input 
                    type="text" placeholder={t.jobDetail.fullName} required
                    className="w-full p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 outline-none focus:ring-2 focus:ring-blue-500 dark:text-white transition-colors"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <select 
                      className="w-full p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 outline-none dark:text-white transition-colors"
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="flex items-center space-x-2">
                      ...
                      <span className="text-sm dark:text-gray-300">{t.jobs.beginnerFriendly}</span>
                    </label>
                    <label className="flex items-center space-x-2">
                      <input type="checkbox" checked={newJob.isMilitary || false} onChange={e => setNewJob({...newJob, isMilitary: e.target.checked})} />
                      <span className="text-sm dark:text-gray-300">{t.jobs.militaryReturnees}</span>
                    </label>
                  </div>
            ...
            <div className="lg:col-span-2 space-y-4">
              {Array.isArray(jobs) && jobs.map(job => (
                <div key={job.id} className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-100 dark:border-gray-800 flex items-center justify-between transition-colors">
                  <div>
        ...
        {activeTab !== 'jobs' && (
          <div className="text-center py-20 bg-gray-50 dark:bg-gray-900 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-800 transition-colors">
            <p className="text-gray-500 dark:text-gray-400">{language === 'en' ? `Management for ${activeTab} coming soon...` : language === 'ru' ? `Управление для ${activeTab} скоро будет доступно...` : `${activeTab} boshqaruvi tez orada qo‘shiladi...`}</p>
          </div>
        )}
      </div>
    </div>
